  ...
  return (
    <section className="space-y-3">
      {weekDays.map((day) => {
        ...
        return (
          <div key={day.key} className="overflow-hidden rounded-lg border bg-white shadow-sm">
            <div className="border-b bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-800">{day.label}</div>
                ...
                dayClasses.map((item) => {
                  ...
                  return (
                    <button
                      key={`${day.key}-${item.id}`}
                      type="button"
                      className="grid w-full gap-2 px-4 py-3 text-left transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 sm:grid-cols-[120px_1fr]"
                      onClick={() => onView(item)}
                    >
