...
export function ClassForm({ initialValue, overlaps, timeOptions, onPreview, onSubmit, onCancel }: ClassFormProps) {
  ...
  const selectableTimes = useMemo(
    () => Array.from(new Set([...timeOptions, ...buildTimeOptions(10)])).sort((a, b) => timeToMinutes(a) - timeToMinutes(b)),
    [timeOptions]
  );
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <Field label="Instructor" error={submitted && errors.instructor ? "Required" : undefined}>
          <Input value={value.instructor} onChange={(event) => update("instructor", event.target.value)} placeholder="Instructor name" />
        </Field>
        <Field label="Room / building" error={submitted && errors.room ? "Required" : undefined}>
          <Input value={value.room} onChange={(event) => update("room", event.target.value)} placeholder="LRC 205" />
        </Field>
        <Field label="Days" error={submitted && errors.days ? "Choose at least one day" : undefined}>
                ...
                <input
                  ...
                  onChange={(event) => {
                    const days = event.target.checked
                      ? [...value.days, day.key]
                      : value.days.filter((selectedDay) => selectedDay !== day.key);
        ...
        <Field label="End time" error={submitted && errors.time ? "End must be after start" : undefined}>
      ...
      {overlaps.length > 0 ? (
        <div className="flex gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
          ...
          <p>
            This class overlaps with {overlaps.map((item) => item.courseCode).join(", ")}. You can still save it; overlapping classes will stack in the timetable cell.
          </p>
        </div>
      ) : null}
